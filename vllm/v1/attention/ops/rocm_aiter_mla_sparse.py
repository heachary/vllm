...
# ==============================
# Split-K decode (ragged) kernels
# ==============================

@triton.jit
def _sparse_attn_decode_ragged_splitk_partial_kernel(
    q_ptr,
    main_cache_ptr,
    main_indices_ptr,
    main_indptr_ptr,
    ...
    tl.store(scratch_m_ptr + base, m_i, mask=head_mask)
    tl.store(scratch_l_ptr + base, l_i, mask=head_mask)

    acc_nope_ptrs = scratch_acc_nope_ptr + base * NOPE_BLOCK + nope_offsets[None, :]
    tl.store(acc_nope_ptrs, acc_nope, mask=head_mask[:, None] & nope_mask[None, :])

    acc_rope_ptrs = scratch_acc_rope_ptr + base * ROPE_DIM + rope_offsets[None, :]
    tl.store(acc_rope_ptrs, acc_rope, mask=head_mask[:, None])


@triton.jit
def _sparse_attn_decode_ragged_splitk_reduce_kernel(
    scratch_m_ptr,
    scratch_l_ptr,
    scratch_acc_nope_ptr,
    scratch_acc_rope_ptr,
    ...
    for s in tl.static_range(0, SPLIT_K):
        base = base0 + s
        m_s = tl.load(scratch_m_ptr + base, mask=head_mask, other=neg_large).to(tl.float32)
        l_s = tl.load(scratch_l_ptr + base, mask=head_mask, other=0.0).to(tl.float32)

        acc_nope_s = tl.load(
            scratch_acc_nope_ptr + base * NOPE_BLOCK + nope_offsets[None, :],
            mask=head_mask[:, None] & nope_mask[None, :],
            other=0.0,
        ).to(tl.float32)
        acc_rope_s = tl.load(
            scratch_acc_rope_ptr + base * ROPE_DIM + rope_offsets[None, :],
            mask=head_mask[:, None],
            other=0.0,
        ).to(tl.float32)
...
def _rocm_sparse_attn_decode_ragged_triton(
    ...
    # Increase grid parallelism for decode: with num_heads=16, BLOCK_H=16 collapses grid to (Q,1).
    # Using BLOCK_H=1 expands grid to (Q,16) and improves occupancy without Split-K scratch overhead.
    block_h = 1
    block_k = 16 if head_dim >= 256 else 32
    out = torch.empty_like(q, dtype=torch.bfloat16)
    ...
    # Disable Split-K for now; rely on head-block parallelism expansion.
    use_splitk = False
    ...
    if use_splitk:
        ...
        scratch_l = torch.empty(
            (num_queries * num_head_blocks * split_k, block_h),
            device=q.device,
            dtype=torch.float32,
        )
        scratch_acc_nope = torch.empty(
            (num_queries * num_head_blocks * split_k, triton.next_power_of_2(nope_head_dim)),
            device=q.device,
            dtype=torch.float32,
        )
        scratch_acc_rope = torch.empty(
            (num_queries * num_head_blocks * split_k, rope_head_dim),
            device=q.device,
            dtype=torch.float32,
        )

        _sparse_attn_decode_ragged_splitk_partial_kernel[
            (num_queries, num_head_blocks * split_k)
        ](
            q,
            main_cache,
            main_indices,
        ...
        _sparse_attn_decode_ragged_splitk_reduce_kernel[(num_queries, num_head_blocks)](
            scratch_m,
            scratch_l,
            scratch_acc_nope,
            scratch_acc_rope,
    ...
    _sparse_attn_decode_ragged_kernel[(num_queries, num_head_blocks)](
        q,
        main_cache,
        main_indices,
        main_indptr,
